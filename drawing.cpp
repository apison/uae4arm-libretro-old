  ...
  if(picasso_on)
	...
	if (y >= TD_PADY && y - TD_PADY < TD_NUM_HEIGHT) 
	{
		...
		else if (track==-10)
		{
			...
			if (second_joystick_enable==0)
			{
				write_tdletter(buf, x + offs, y - TD_PADY, 'M');
				write_tdletter(buf, x + offs + TD_NUM_WIDTH, y - TD_PADY, 'M');
			}
			else
			{
				write_tdletter(buf, x + offs, y - TD_PADY, 'J');
				write_tdletter(buf, x + offs + TD_NUM_WIDTH, y - TD_PADY, 'J');
			}
	  ...
	  if (savestate_state == STATE_DORESTORE) {
	    ...
	    check_picasso ();
		  ...
		  if (check_prefs_changed_gfx ()) {
			//write_log ("Drawing.cpp vsync_handle_redraw reset_drawing . \n");
			reset_drawing ();
			//write_log ("Drawing.cpp vsync_handle_redraw notice_new_xcolors . \n");
			notice_new_xcolors ();
		  }

//write_log ("Drawing.cpp vsync_handle_redraw check_prefs_changed_audio . \n");
			
		  check_prefs_changed_audio ();
//write_log ("Drawing.cpp vsync_handle_redraw check_prefs_changed_custom . \n");
		  check_prefs_changed_custom ();
//write_log ("Drawing.cpp vsync_handle_redraw check_prefs_changed_cpu . \n");
		  check_prefs_changed_cpu ();
//write_log ("Drawing.cpp Frame : %d. \n",fs_framecnt);
